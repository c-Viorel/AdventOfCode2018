...
//  AdventOfCode2018
//
//  Created by Viorel Porumbescu on 24/12/2018.
...
class Day24{
    ...
    func showSolutions() {
        ...
        do {
            let (imm, inf) = aocD24(immune: immune.map { $0.copy() }, infection: infection.map { $0.copy() })
            print("""
                \tA: \(inf.lazy.map({ $0.num }).reduce(0, +))
                """)
        }
        ...
        for boost in 2500... {
            ...
            let (imm, inf) = aocD24(immune: newImmune, infection: infection.map { $0.copy() })
            if !imm.isEmpty && inf.isEmpty {
                print("\tB: \(imm.lazy.map({ $0.num }).reduce(0, +))")
                break
            }
        }
        if printRuntimes {
    ...
    func aocD24(immune: [Unit], infection: [Unit]) -> (immune: [Unit], infection: [Unit]) {
        var immune = immune
